...
import { resolveSubagentStatusPluginOptions } from '../../../src/features/subagent-status/runtime/options.ts';
import {
...
describe('refresh runtime', () => {
  ...
  it('waits for recovery hydration during refresh before applying client snapshots', async () => {
    ...
    vi.doMock('../../../src/features/subagent-status/infrastructure/sqlite/hydrate.ts', () => ({
      createSQLiteRecoverySource: vi.fn(() => ({
    ...
    const runtime = createTuiRuntime(
      ...
      {
        debug: false,
        recovery: { sqliteDatabasePath: ':mock:' },
  ...
  it('keeps SQLite terminal recovery authoritative over newer TUI and client running snapshots during refresh', async () => {
    ...
    const tempDir = await mkdtemp(join(tmpdir(), 'agent-monitor-'));
  ...
  it('bootstraps setDebugEnabled(true) from debug: true plugin option', async () => {
    ...
    const debugModule = await import('../../../src/features/subagent-status/shared/display.ts');
