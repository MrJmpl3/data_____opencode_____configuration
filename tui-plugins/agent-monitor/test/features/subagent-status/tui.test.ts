...
import { resolveElapsedMs } from '../../../src/features/subagent-status/domain/state/core.ts';
import { buildTuiSnapshot } from '../../../src/features/subagent-status/runtime/snapshot.ts';
import {
...
import { persistSnapshot } from '../../../src/features/subagent-status/infrastructure/persistence.ts';
...
describe('tui elapsed time', () => {
  ...
  it('persists the rendered status line for the current visible snapshot', async () => {
    ...
    const statePath = join(await mkdtemp(join(tmpdir(), 'agent-monitor-')), 'state.json');
  ...
  it('binds row navigation to mouse release instead of mouse press', () => {
    const source = readFileSync(new URL('../../../src/features/subagent-status/ui/view.tsx', import.meta.url), 'utf8');
