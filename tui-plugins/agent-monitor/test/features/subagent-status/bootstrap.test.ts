...
describe('tui bootstrap buffering', () => {
  it('buffers early bridge events until the initial snapshot write completes', async () => {
        ...
        createPersistQueue: vi.fn(
          (..._args: [string, string, unknown]) =>
            async (state: { children: Record<string, unknown> }, _meta: unknown) => {
              await saveState('/tmp/agent-monitor-state.json', state);
              await saveStatusText('/tmp/agent-monitor-status.txt', '');
            },
        ),
    ...
    const { registerSubagentStatusTui } = await import('../../../src/features/subagent-status/runtime/runtime.tsx');
  ...
  it('accepts explicit plugin tuple options and normalizes them before bootstrap', async () => {
    ...
    vi.doMock('../../../src/features/subagent-status/infrastructure/sqlite/hydrate.ts', () => ({
      createSQLiteRecoverySource,
    }));

    const { registerSubagentStatusTui } = await import('../../../src/features/subagent-status/runtime/runtime.tsx');
    const { DEFAULT_STALE_RUNNING_PROBE_POLICY } =
      await import('../../../src/features/subagent-status/runtime/options.ts');
    ...
    await registerSubagentStatusTui(api, {
