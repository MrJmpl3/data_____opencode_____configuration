...
import { loadState, persistSnapshot } from '../../src/infrastructure/persistence.ts';
...
describe('persistence round-trip integration', () => {
  ...
  it('preserves children, counts, totalExecuted, and updatedAt through save-load round-trip', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'mrjmpl3-subagent-status-roundtrip-'));
    tempDirs.push(dir);
  ...
  it('survives a second save-load cycle without data loss', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'mrjmpl3-subagent-status-roundtrip2-'));
    tempDirs.push(dir);
    const statePath = join(dir, 'state.json');
    const textPath = join(dir, 'status.txt');
