...
import { readSQLiteRecoveryRows, type SQLiteRecoveryRow } from '../../src/infrastructure/recovery/sqlite.ts';
...
describe('SQLite recovery integration', () => {
  ...
  it('reads and classifies seeded SQLite recovery rows correctly', async () => {
    ...
    const tempDir = mkdtempSync(join(tmpdir(), 'mrjmpl3-subagent-sqlite-'));
